...
Pipelining: agents from the next round start as soon as they have
all positions from the previous round, overlapping with slower agents.
"""
...
def run_debate_round(state: ConsensusState, llm: ChatOpenAI) -> dict:
    ...
    round_id = f"round_{current_round}_{id(state)}"
    zeus_begin(round_id)
...
def summarize(state: ConsensusState, llm: ChatOpenAI) -> dict:
    ...
    sum_id = f"summarize_{id(state)}"
    zeus_begin(sum_id)
    response = llm.invoke(messages)
    energy = zeus_end(sum_id)
...
# ---------------------------------------------------------------------------
# Multi-panel consensus: run multiple concurrent consensus groups
...
def run_multi_panel(
    ...
    """Run multiple consensus panels concurrently to saturate the GPU."""
    import threading
    ...
    def run_panel(panel_id, topic):
        ...
        with lock:
            results[panel_id] = {
                ...
                "timings": result["round_timings"],
                "summary": result["final_summary"],
            }
        print(f"\n[Panel {panel_id}] Done in {elapsed:.1f}s, "
              f"consensus={result['consensus_reached']}, "
              f"rounds={result['current_round']}")

    threads = []
    for i, topic in enumerate(topics):
        t = threading.Thread(target=run_panel, args=(i, topic))
        threads.append(t)
        t.start()
...
if __name__ == "__main__":
    ...
    if args.multi_panel:
        # Run 3 concurrent panels to saturate GPU
        topics = [
            ...
            "How should a living model handle conflicting memories — when new observations contradict stored knowledge? Should it favor recency, confidence, source authority, or consensus? What data structures support efficient conflict resolution at scale?",
            "What is the right security and governance model for an AI system that continuously learns? How do you audit what it knows, when it learned it, and roll back unsafe updates without losing valid knowledge acquired in the same time window?",
        ]
        zeus_begin("multi_panel_full")
        t0 = time.perf_counter()

        results = run_multi_panel(
            topics=topics,
        ...
        for pid, r in sorted(results.items()):
            reqs = r["rounds"] * args.num_agents + 1
            total_reqs += reqs
            print(f"  Panel {pid}: {r['rounds']} rounds, consensus={r['consensus']}, "
                  f"{r['time_s']:.1f}s, {reqs} reqs")

        print(f"\n  Total wall time: {elapsed:.1f}s")
        print(f"  Total requests across all panels: {total_reqs}")
        ...
        outfile = f"results_multipanel_{int(time.time())}.json"
        with open(outfile, "w") as f:
            json.dump({
                "mode": "multi_panel",
                "panels": results,
